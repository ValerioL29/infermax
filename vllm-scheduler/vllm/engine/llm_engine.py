...
class LLMEngine:
    ...
    def validate_output(
        ...
        if (TYPE_CHECKING or do_validate) and not isinstance(output, output_type):
            raise TypeError(
                f"Expected output of type {output_type}, but found type {type(output)}"
            )
    ...
    def __init__(
        ...
        def get_tokenizer_for_seq(sequence: Sequence) -> AnyTokenizer:
            assert (
                tokenizer_group
            ), "tokenizer_group cannot be None, make sure skip_tokenizer_init is False"
        ...
        # Set the number of total GPU cache
        self.precomputed_schedule.num_total_gpu_cache = self.cache_config.num_gpu_blocks
    ...
    def add_request(
        ...
        *,
        ...
        if priority != 0 and not self.scheduler_config.policy == "priority":
            raise ValueError(
                f"Got priority {priority} but Priority scheduling is not enabled."
            )

        if arrival_time is None:
            if self.scheduler_config.use_zero_cpu_scheduling:
                arrival_time = self.precomputed_schedule.accumulated_gpu_time
            else:
    ...
    def _process_model_outputs(
        self, ctx: SchedulerContext, request_id: Optional[str] = None
    ) -> None:
        ...
        # print('outputs_by_sequence_group')
        # print(outputs_by_sequence_group)
        # print('skip', skip)
        ...
        # print('has_multiple_outputs', has_multiple_outputs)
        ...
        for i in indices:
            ...
            if has_multiple_outputs:
                ...
                # print('sequence id', i, indices, len(outputs_by_sequence_group))
                # print(seq_group_meta)
                # print(scheduled_seq_group)
                # print(outputs_by_sequence_group[0][i])

                try:
                    output = [outputs_by_sequence_group[0][i]]
                except:
                    seq_group_output = CompletionSequenceGroupOutput(
                        samples=[], prompt_logprobs=None
                    )
                    output = [seq_group_output]
                    # output = [outputs_by_sequence_group[0][0]]
                # else:
        ...
        for i in finished_now:
            ...
            request_output = RequestOutputFactory.create(
                seq_group, self.seq_id_to_seq_group, use_cache=self.use_cached_outputs
            )
            if request_output:
        ...
        for seq_group in scheduler_outputs.ignored_seq_groups:
            ...
            if (
                params is not None
                and params.output_kind == (RequestOutputKind.DELTA)
                and not seq_group.is_finished()
            ):
                continue
    ...
    def _advance_to_next_step(
        ...
        for seq_group_metadata, sequence_group_outputs, scheduled_seq_group in zip(
            ...
            if self.scheduler_config.is_multi_step:
                # Updates happen only if the sequence is prefill
                self._update_num_computed_tokens_for_multi_step_prefill(
                    seq_group, seq_group_metadata, seq_group.state.num_steps == 1
                )
            else:
                token_chunk_size = (
                    seq_group_metadata.token_chunk_size
                    if seq_group_metadata.token_chunk_size is not None
                    else 0
                )
    ...
    def step(self) -> List[Union[RequestOutput, EmbeddingRequestOutput]]:
        ...
        if not scheduler_outputs.is_empty():
            ...
            self.precomputed_schedule.model_execution_time[current_step] = (
    ...
    def _has_remaining_steps(
        ...
        # print('_has_remaining_steps', self.scheduler_config.is_multi_step, seq_group_metadata_list)
        ...
        # print('ref_remaining_steps', ref_remaining_steps)
    ...
    def _update_cached_scheduler_output(
        self, virtual_engine: int, output: List[Optional[SamplerOutput]]
    ) -> None:
        if (
    ...
    def do_log_stats(
        ...
        if self.log_stats:
            stats = self._get_stats(
                scheduler_outputs, model_output, finished_before, skip
            )
    ...
    def _get_stats(
        ...
        cpu_prefix_cache_hit_rate = self.scheduler[0].get_prefix_cache_hit_rate(
            Device.CPU
        )
        gpu_prefix_cache_hit_rate = self.scheduler[0].get_prefix_cache_hit_rate(
            Device.GPU
        )
    ...
    def _validate_model_inputs(
        self, inputs: Union[DecoderOnlyInputs, EncoderDecoderInputs]
    ):
    ...
    def _build_logits_processors(
        ...
        if (guided_decoding := sampling_params.guided_decoding) is not None:
            ...
            logger.debug(
                "Building guided decoding logits processor in LLMEngine. Params: %s",
                guided_decoding,
            )

            tokenizer = self.get_tokenizer(lora_request=lora_request)
            guided_decoding.backend = (
                guided_decoding.backend or self.decoding_config.guided_decoding_backend
            )
