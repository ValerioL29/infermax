...
class ModelInputForGPUBuilder(ModelRunnerInputBuilderBase[ModelInputForGPU]):
    ...
    def _compute_lens(
        ...
        if inter_data.is_prompt:
            ...
            or self.runner.model_config.is_encoder_decoder_model
        ):
    ...
    def _compute_multi_modal_input(
        ...
        mm_kwargs = self.multi_modal_input_mapper(
            mm_data, mm_processor_kwargs=seq_group_metadata.mm_processor_kwargs
        )
...
class GPUModelRunnerBase(ModelRunnerBase[TModelInputForGPU]):
            ...
            for virtual_engine in range(self.parallel_config.pipeline_parallel_size):
                for batch_size in reversed(batch_size_capture_list):
                    attn_metadata = self.attn_state.graph_capture_get_metadata_for_batch(
                        batch_size,
                        is_encoder_decoder_model=self.model_config.is_encoder_decoder_model,
                    )
    ...
    def _update_inputs_to_capture_for_enc_dec_model(
        ...
        capture_inputs["encoder_input_ids"] = torch.tensor([], dtype=torch.long).cuda()
        capture_inputs["encoder_positions"] = torch.tensor([], dtype=torch.long).cuda()
...
class ModelRunner(GPUModelRunnerBase[ModelInputForGPUWithSamplingMetadata]):
    ...
    def execute_model(
        ...
        with set_forward_context(model_input.attn_metadata):
            model_forward_start_m = torch.cuda.Event(enable_timing=True)
            model_forward_end_m = torch.cuda.Event(enable_timing=True)
            model_forward_start_m.record()
