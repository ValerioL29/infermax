...
class PrecomputedSchedule(Singleton):

    def __init__(self):
        # Initialize scheduler-specific attributes here
...
class Scheduler:
    ...
    def __init__(
        ...
        self.artificial_preempt_cnt = (
            ARTIFICIAL_PREEMPTION_MAX_CNT if self.enable_artificial_preemption else 0
        )
    ...
    def has_unfinished_seqs(self) -> bool:
        return (
            len(self.waiting) != 0 or len(self.running) != 0 or len(self.swapped) != 0
        )
    ...
    def _schedule_running_infermax(
        self,
        ...
        running_seq_groups = list(self.running)
        remaining_seq_groups = []

        # print('evict', self.batch_id, self.precomputed_schedule.evicted)
        evicted = self.precomputed_schedule.evicted[self.batch_id]

        for seq_group in running_seq_groups:
            if int(seq_group.request_id) not in evicted:
                remaining_seq_groups.append(seq_group)
                continue
        ...
        running_seq_groups = remaining_seq_groups
        remaining_seq_groups = []

        c = self.precomputed_schedule.c[self.batch_id]
        for seq_group in running_seq_groups:

            num_running_tokens = c.get(int(seq_group.request_id), 0)
            ...
            if not self._can_append_slots(seq_group, enable_chunking):
                assert False
        ...
        self.running = deque(remaining_seq_groups)
    ...
    def _schedule_swapped_infermax(
        self,
        budget: SchedulingBudget,
        curr_loras: Optional[Set[int]],
        enable_chunking: bool = False,
        ...
        swapped_seq_groups = list(self.swapped)
        remaining_seq_groups = []

        c = self.precomputed_schedule.c[self.batch_id]
        for seq_group in swapped_seq_groups:

            if c.get(int(seq_group.request_id), 0) == 0:
                remaining_seq_groups.append(seq_group)
                continue

            is_prefill = seq_group.is_prefill()
            alloc_status = self.block_manager.can_swap_in(
                seq_group, self._get_num_lookahead_slots(is_prefill, enable_chunking)
            )
            ...
            num_new_tokens = c.get(int(seq_group.request_id), 0)
            ...
            if is_prefill:
                prefill_seq_groups.append(
                    ScheduledSequenceGroup(seq_group, token_chunk_size=num_new_tokens)
                )
            else:
        ...
        self.swapped = deque(remaining_seq_groups)
    ...
    def _schedule_priority_preemption(
        ...
        if waiting_queue:
            ...
            while running_queue and self._get_priority(
                ...
                budget.subtract_num_batched_tokens(
                    vseq_group.request_id, num_running_tokens
                )
    ...
    def _schedule_prefills_infermax(
        self,
        budget: SchedulingBudget,
        curr_loras: Optional[Set[int]],
        enable_chunking: bool = False,
        ...
        waiting_seq_groups: List[SequenceGroup] = list(self.waiting)
        remaining_seq_groups: List[SequenceGroup] = []

        leftover_waiting_sequences: Deque[SequenceGroup] = deque()

        c = self.precomputed_schedule.c[self.batch_id]

        for seq_group in waiting_seq_groups:
            if c.get(int(seq_group.request_id), 0) == 0:
                remaining_seq_groups.append(seq_group)
                continue
            ...
            num_new_tokens = c.get(int(seq_group.request_id), 0)
            # print('_schedule_prefills_infermax', self.batch_id, seq_group.request_id, self.precomputed_schedule.p[self.batch_id], num_new_tokens, enable_chunking, self.scheduler_config.is_multi_step)
        ...
        self.waiting = deque(remaining_seq_groups)
        if len(seq_groups) > 0:
    ...
    def _schedule_infermax(self, enable_chunking) -> SchedulerOutputs:
        ...
        running_scheduled = self._schedule_running_infermax(
            budget, curr_loras, enable_chunking=enable_chunking
        )

        swapped_in = self._schedule_swapped_infermax(budget, curr_loras)
        # Schedule new prefills.
        prefills = self._schedule_prefills_infermax(
            budget, curr_loras, enable_chunking=enable_chunking
        )
    ...
    def _schedule(self) -> SchedulerOutputs:
        ...
        if self.use_infermax_schedule:
            return self._schedule_infermax(
                self.scheduler_config.chunked_prefill_enabled
            )
        else:
        ...
        if enable_chunking and len(seqs) == 1:
            ...
            if self.scheduler_config.is_multi_step:
                ...
                if remaining_token_budget < num_new_tokens:
                    num_new_tokens = (remaining_token_budget // block_size) * block_size
            else:
