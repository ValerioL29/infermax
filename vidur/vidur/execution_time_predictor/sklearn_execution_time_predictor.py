...
if model_7B:
    ...
    # prefill-attn from self
    a_p_dict = {0: 4.309e-08}
    b_p_dict = {0: 0.2112} # decrease to match vllm and sarathi?
    a_d = 1.222e-06
    b_d = 0.081
    a_o = 0.002
    b_o = 0.159
...
infermax = True #XXX: attention only
infermax_prefill = True
infermax_decode  = True
infermax_other   = True #XXX: change later
infermax_kernel  = True
infermax_tensor  = True

infermax_prefill_m_read = True
infermax_prefill_mc     = True
...
class SklearnExecutionTimePredictor(BaseExecutionTimePredictor):
    ...
    def _load_attention_df(self, file_path: str) -> pd.DataFrame:
        ...
        #XXX
        assert len(ret) > 0, f'No data found for model {self._model_config.get_name()} and block size {self._block_size} in {file_path}'
    ...
    def _get_model_hash(self, model_name: str, df: pd.DataFrame = None) -> str:
        ...
        #print('get_model_hash original config', self_dict)
        #print('get_model_hash model_name', model_name)
        if False:
            #XXX: select RF and LR
            if model_name == 'attn_prefill' or "RandomForest" in str(model_name):
                print('get_model_hash SET RF')
                self_dict['model_provider'] = 'random_forrest'
            else:
                print('get_model_hash SET LINEAR')
                self_dict['model_provider'] = 'linear_regression'
        config_str = str(self_dict)
        #print('get_model_hash new config', config_str)
        ...
        #print('get_model_hash df_hash_str', df_hash_str)
    ...
    def _train_model(
        ...
        #assert False, f'cannot find cached model {self.to_dict()}'
    ...
    def _get_model_prediction(
        ...
        #XXX: sending "model" is weird... perhaps model_name???
        ...
        #assert False, f'cannot find cached predictions {self.to_dict()}'
    ...
    def _train_compute_models(self) -> Dict[str, BaseEstimator]:
        ...
        #print(f"attention_df: {attention_df}")
        attention_df = self._get_attention_df_with_derived_features(attention_df)
        #print(f"attention_df after derived features: {attention_df}")
        ...
        for model_name in model_names:
            #XXX now len can be 0 for block_size = 1
            if len(attention_df) == 0:
                #assert False, f"attention_df is empty for {model_name}"
                continue
            models[model_name] = self._train_model(
                model_name=model_name,
                df=attention_df,
    ...
    def _train_attention_layer_models(self) -> Dict[str, BaseEstimator]:
        ...
        if len(prefill_df) > 0:
            ...
            print('no prefill_df for attn_prefill')
        ...
        if len(decode_df) > 0:
            ...
            print('no decode_df for attn_decode')
    ...
    def _predict_for_compute_models(self) -> Dict[str, Any]:
        ...
        for model_name in model_names:
            ...
            if model_name in self._models:
                ...
                logger.error(f"model_name {model_name} is not in self._models")
    ...
    def _predict_for_attention_layer_models(self) -> Dict[str, Any]:
        ...
        #XXX
        if 'attn_prefill' in self._models:
        ...
        #XXX
        if 'attn_decode' in self._models:
    ...
    def _predict_from_models(self) -> Dict[str, Any]:
        ...
        #XXX: load attention
        if not (infermax or infermax_prefill):
            #assert False
            predictions.update(self._predict_for_attention_layer_models()) #XXX: bottleneck, but then possibly missing attn_kv_cache_save
        #else:
        #    assert False

        #assert 'attn_prefill' in predictions
    ...
    def _get_batch_decode_attention_params(self, batch: Batch) -> Tuple[int, int]:
        ...
        if infermax:
            decode_avg_kv_cache_size = np.mean(decode_kv_cache_sizes)
        else:
    ...
    def _get_tensor_parallel_communication_time(self, batch: Batch) -> float:
        if infermax_tensor:
            return 0
            return (
                a_m * batch._total_num_tokens_rounded + b_m
    ...
    def _get_pipeline_parallel_communication_time(self, batch: Batch) -> float:
        try:
            ...
            logger.error(f"Failed to get send_recv prediction for batch {batch} where keys are {self._predictions['send_recv'].keys()}")
            raise e
    ...
    def _get_attention_rope_execution_time(self, batch: Batch) -> float:
        try:
            ...
            logger.error(f"Failed to get attn_rope prediction for batch {batch} where keys are {self._predictions['attn_rope'].keys()}")

    def _get_attention_kv_cache_save_execution_time(self, batch: Batch) -> float:
        #assert not infermax_other
        ...
        #if infermax: #XXX: this can be wrong, 0 for Llama 2 but > 0 for Llama 3
        if infermax:
            return 0
        else:
            assert False
            try:
                ...
                logger.error(f"Failed to get attn_kv_cache_save prediction for batch {batch} where keys are {self._predictions.keys()}")
                #XXX
                #return 0
                raise e
    ...
    def _get_attention_decode_execution_time(self, batch: Batch) -> float:
        ...
        if infermax or infermax_decode:
            #if infermax_kernel:
            #    ret = a_d * decode_batch_size * decode_avg_kv_cache_size + b_d + s_d * decode_batch_size
            #else:
            ret = (a_d * decode_batch_size * decode_avg_kv_cache_size + b_d) * (
                1
                + self._attention_decode_batching_overhead_fraction
                * int(decode_batch_size > 1)
            )
            if infermax_kernel:
                ret += s_d * decode_batch_size
            return ret
        else:
            #XXX:
            if (decode_batch_size, decode_avg_kv_cache_size) not in self._predictions["attn_decode"]:
                #print('attn_decode keys', self._predictions['attn_decode'].keys())
                print('max attn_decode key', list(self._predictions['attn_decode'].keys())[-1])
    ...
    def _get_attention_prefill_execution_time(self, batch: Batch) -> float:
        ...
        assert len(prefill_params) == len(kv_cache_sizes) == len(prefill_chunk_sizes), f'{len(prefill_params)} == {len(kv_cache_sizes)} == {len(prefill_chunk_sizes)}'
        agg_prefill_chunk_dot_kv_size = sum([x * y for x, y in prefill_params])

        #XXX: replace attn_prefill with vidur's
        if infermax or infermax_prefill:
            #assert False
            import bisect
            keys = list(a_p_dict.keys())
            idx = bisect.bisect_right(keys, agg_kv_cache_size)
            ...
            #print('# KV in attn_prefill', agg_kv_cache_size, 'idx', idx - 1)

            #if infermax_kernel:
            #    ret = (a_p * (agg_prefill_chunk_size ** 2) + b_p) + (kv_p * agg_kv_cache_size) + (s_p * len(prefill_params))
            #else:
            if infermax_prefill_mc:
                compute = (a_p * (agg_prefill_chunk_size ** 2 + 2 * agg_prefill_chunk_dot_kv_size) + b_p)
            else:
                compute = (a_p * (agg_prefill_chunk_size ** 2) + b_p)
            ret = compute * (
                1
                + self._attention_prefill_batching_overhead_fraction
            ...
            if infermax_kernel:
                ret += s_p * len(prefill_params)
            if infermax_prefill_m_read:
                ret += m_p * agg_kv_cache_size
            return ret
        else:
