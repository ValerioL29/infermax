...
class BaseReplicaScheduler(ABC):
    def __init__(
        ...
        print('FINAL MAX BATCH SIZE', self._max_batch_size)
    ...
    def allocate(self, request_id: int, num_blocks: int) -> None:
        ...
        assert self._num_allocated_blocks <= self._config.num_blocks, f"num_allocated_blocks: {self._num_allocated_blocks}, num_blocks: {self._config.num_blocks}"

    def free(self, request_id: int, x: float = 1.0) -> int:
        num_blocks = self._allocation_map.pop(request_id)
        if x <= 1:
            num_blocks_to_free = min(num_blocks, int(ceil(x * num_blocks)))
        else:
            num_blocks_to_free = min(x, num_blocks)
        self._num_allocated_blocks -= num_blocks_to_free
        if num_blocks_to_free < num_blocks:
            self._allocation_map[request_id] = (num_blocks - num_blocks_to_free)
            assert self._allocation_map[request_id] > 0
        #print('[free] request', request_id, 'free', num_blocks_to_free, '/', num_blocks, 'remaining', (num_blocks - num_blocks_to_free) * self._config.block_size)
        assert self._num_allocated_blocks >= 0
        return int((num_blocks - num_blocks_to_free) * self._config.block_size)
