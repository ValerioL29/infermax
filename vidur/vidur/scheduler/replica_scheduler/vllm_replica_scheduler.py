    ...
    def _get_next_batch(self) -> Batch:
        ...
        while self._request_queue:
            ...
            if not self._can_allocate_request(request):
                #print('CAN NOT ALLOCATE', request.id)
                break

            new_num_tokens = num_tokens + [next_num_tokens]
            #XXX not using cuda graph
            new_num_batch_tokens = sum(new_num_tokens) #len(new_num_tokens) * max(new_num_tokens)
            if new_num_batch_tokens > self._config.max_tokens_in_batch:
                #print('C_LIMIT', request.id)
                break

            if len(self._allocation_map) == self._config.batch_size_cap:
                #print('R_LIMIT', request.id)
                break

            if len(requests) == self._max_micro_batch_size:
                #print('B_LIMIT', request.id)
                break
        ...
        #XXX hybrid batch
        if requests and not self._config.hybrid_batch:
        ...
        while self._preempted_requests:
            ...
            #XXX added
            if num_batch_tokens >= self._config.max_tokens_in_batch:
                #print('C_LIMIT', request.id)
                break
            ...
            while not self._can_allocate_request(request):
                if self._preempted_requests:
                    ...
                    #print('EVICT VICTIM', victim_request.id)
                else:
                    request.restart()
                    self.free(request.id)
                    self._request_queue = [request] + self._request_queue
                    #print('EVICT SELF', request.id)
                    break
            else:
                self._allocate_request(request)
                next_num_tokens = self._get_request_next_num_tokens(request)
