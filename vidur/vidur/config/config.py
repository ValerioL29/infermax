...
class BaseReplicaSchedulerConfig(BasePolyConfig):
    ...
    no_evict: bool = field(
        default=False,
        metadata={"help": "Whether to run scheduler in the eviction-free mode."},
    )
    sortI: bool = field(
        default=False,
        metadata={"help": "Whether to sort requests by num_prefill_tokens."},
    )
    histogram: bool = field(
        default=False,
        metadata={"help": "Whether to use histogram for request allocation."},
    )
    histogram_threshold: int = field(
        default=0,
        metadata={"help": "Threshold for determining short requests when using histogram."},
    )
    overload: bool = field(
        default=False,
        metadata={"help": "Whether to decouple memory and progress."},
    )
    overload_threshold: int = field(
        default=0,
        metadata={"help": "Threshold for total overloading tokens that will not be stored in memory."},
    )
    overload_prefill_threshold: int = field(
        default=0,
        metadata={"help": "Threshold for prefill tokens o candidate requests for overloading."},
    )
    partial_evict_block: float = field(
        default=1.0,
...
class VllmSchedulerConfig(BaseReplicaSchedulerConfig):
    ...
    #XXX  for hybrid batching
    hybrid_batch: bool = field(
        default=False,
        metadata={"help": "Whether to enable hybrid batch."},
    )
...
class SarathiSchedulerConfig(BaseReplicaSchedulerConfig):
    ...
    #XXX  for chunked prefill
    no_chunked_prefill: bool = field(
        default=False,
        metadata={"help": "Whether to disable chunked prefill."},
    )

    #XXX  for no hybrid batching
    no_hybrid_batch: bool = field(
        default=False,
        metadata={"help": "Whether to disable hybrid batch."},
    )
...
@dataclass
class TestSchedulerConfig(BaseReplicaSchedulerConfig):
    chunk_size: int = field(
        default=512,
        metadata={"help": "Chunk size for Sarathi."},
    )
    ...
    @staticmethod
    def get_type():
        return ReplicaSchedulerType.TEST


@dataclass
class RankSchedulerConfig(BaseReplicaSchedulerConfig):
    ...
    @staticmethod
    def get_type():
        return ReplicaSchedulerType.RANK
...
class MetricsConfig:
    ...
    #XXX  for CSP
    store_csp: bool = field(
        default=False,
        metadata={"help": "Whether to store variables for CSP initial seed."},
    )
    #XXX  for vLLM
    store_schedule: bool = field(
        default=False,
        metadata={"help": "Whether to store scheule for input to vLLM."},
    )
