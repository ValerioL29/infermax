...
class ExecutionTime(BaseEntity):
    def __init__(
        ...
        self._other_time = other_time #XXX
        #assert other_time > 0
        ...
        #print('setting pipeline in ExecutionTime', self._pipeline_parallel_communication_time)
    ...
    def _get_mlp_layer_execution_time(self) -> float:
        #XXX: all_reduce ratio inside mlp
        #print('all_reduce ratio in MLP', self._tensor_parallel_communication_time / (
        ...
        #    + self._mlp_norm_time
        #))
        #XXX: defer all to attention
        if self._other_time > 0:
            return (
                self._tensor_parallel_communication_time
            )
        else:
    ...
    def _get_attention_layer_execution_time(self) -> float:
        #print('all_reduce ratio in attention', self._tensor_parallel_communication_time / (
        #    self._attention_layer_pre_proj_execution_time
        #    + self._attention_layer_post_proj_execution_time
        ...
        if self._other_time > 0:
            return (
                ...
                + self._attention_prefill_execution_time
                + self._tensor_parallel_communication_time
                + self._other_time #XXX: including cache save
            )
        else:
            return (
                self._attention_layer_pre_proj_execution_time
    ...
    def model_time(self) -> float:
        ...
        #print('block_execution_time', block_execution_time)
        #print('num_layers', self._num_layers_per_pipeline_stage)
        #print('pipeline', self.pipeline_parallel_communication_time)
        #print('pipeline ratio', self.pipeline_parallel_communication_time / pipeline_stage_execution_time)
        # return in seconds
        return (
