...
class SyntheticRequestGenerator(BaseRequestGenerator):
    ...
    def _generate_next_request(self, last_arrived_at: float) -> Request:
        ...
        #print('inter_request_time', inter_request_time)
        if inter_request_time is None:
            return None
        arrived_at = last_arrived_at + inter_request_time
        #XXX
        #print('arrived_at:', last_arrived_at, inter_request_time, arrived_at)
    ...
    def _generate_requests(self) -> List[Request]:
        ...
        #XXX: added 0
        elif self.config.num_requests is not None and self.config.num_requests > 0:
            #assert False
            ...
            while True:
                ...
                current_time = request.arrived_at
                requests.append(request)
                #print('add request', len(requests), request.num_prefill_tokens)
